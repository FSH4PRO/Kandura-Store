...
<div class="navbar-nav-right d-flex align-items-center" id="navbar-collapse">
  ...
  <div class="navbar-nav align-items-center position-relative">
    <div class="nav-item d-flex align-items-center position-relative" id="navbar-search-container">
      <i class="icon-base bx bx-search icon-md"></i>
      <form action="{{ route('admin.search.index') }}" method="GET" class="d-flex align-items-center w-100"
        id="navbar-search-form">
        <input type="text" name="q" id="navbar-search-input"
          class="form-control border-0 shadow-none ps-1 ps-sm-2" placeholder="{{ __('navbar.search_placeholder') }}"
          aria-label="{{ __('navbar.search_placeholder') }}" autocomplete="off" value="{{ request('q') }}">
      </form>
      {{-- Search suggestions dropdown --}}
      <div class="dropdown-menu dropdown-menu-end w-100 position-absolute top-100 start-0 mt-2 d-none"
        id="search-suggestions" style="max-width: 400px; max-height: 400px; overflow-y: auto;">
        <div class="px-3 py-2 text-muted small">
          <i class="bx bx-search me-1"></i> {{ __('navbar.search_suggestions') }}
        </div>
