                    ...
                    <form method="GET" action="{{ route('admins.index') }}" class="row g-3 align-items-end">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" name="search" class="form-control"
                                placeholder="{{ __('admins.filters.search_placeholder') }}"
                        ...
                        <div class="col-md-2">
                            ...
                            <select name="per_page" class="form-select">
                                @foreach ([10, 25, 50, 100] as $num)
                                    <option value="{{ $num }}"
                                        {{ ($filters['per_page'] ?? 10) == $num ? 'selected' : '' }}>
                                        {{ $num }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <tbody>
                            ...
                            @forelse($admins as $user)
                                @php
                                    ...
                                    $roleNames = collect();
                                    if (
                                        $adminModel instanceof \App\Models\Admin &&
                                        method_exists($adminModel, 'getRoleNames')
                                    ) {
                                    ...
                                    $targetIsSuper =
                                        $adminModel &&
                                        ($adminModel->super_admin || $adminModel->hasRole('super_admin'));

                                    $canDelete =
                                        $actorIsSuper &&
                                        $adminModel &&
                                        $actor->id !== $adminModel->id &&
                                        !$targetIsSuper;
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($user->is_active)
                                            <span
                                                class="badge bg-label-success">{{ __('admins.status_badge.active') }}</span>
                                        @else
                                            <span
                                                class="badge bg-label-danger">{{ __('admins.status_badge.inactive') }}</span>
                                        @endif
                                    </td>
