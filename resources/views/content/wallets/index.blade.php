      ...
      <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
        ...
        <div>
          @can('bulkAddCredit', App\Models\Wallet::class)
            <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#bulkTopupModal">
          ...
          <form method="GET" action="{{ route('admin.wallets.index') }}" class="row g-3 align-items-end">
            ...
            <div class="col-md-3">
              ...
              <input type="text" name="search" class="form-control"
                placeholder="{{ __('wallets.filters.search_placeholder') }}" value="{{ $filters['search'] ?? '' }}">
            </div>
                  ...
                  <td class="text-center">
                    <div class="dropdown">
                      ...
                      <div class="dropdown-menu dropdown-menu-end">
                        <a href="{{ route('admin.wallets.show', $wallet->id) }}" class="dropdown-item">
                          ...
                          @if ($wallet->is_active)
                            <form action="{{ route('admin.wallets.deactivate', $wallet->id) }}" method="POST"
                              ...
                              <button type="submit" class="dropdown-item text-warning">
                                <i class="bx bx-x me-2"></i> {{ __('wallets.table.deactivate') }}
                              </button>
                            </form>
                          @else
                            <form action="{{ route('admin.wallets.activate', $wallet->id) }}" method="POST">
                              ...
                              <button type="submit" class="dropdown-item text-success">
                                <i class="bx bx-check me-2"></i> {{ __('wallets.table.activate') }}
                              </button>
                            </form>
                          @endif
                        @endcan
          ...
          <div class="modal-content">
            <form action="{{ route('admin.wallets.topup', $wallet->id) }}" method="POST">
              ...
              <div class="modal-header">
                <h5 class="modal-title">{{ __('wallets.modal.topup_title') }} -
                  {{ $wallet->customer->user->name ?? $wallet->customer->phone }}</h5>
