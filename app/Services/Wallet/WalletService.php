...
use App\Notifications\User\WalletTransactionNotification;
...
class WalletService
{
    ...
    public function credit(Wallet $wallet, float $amount, string $description = 'Top up', array $meta = []): WalletTransaction
    {
        ...
        $transaction = DB::transaction(function () use ($wallet, $amount, $description, $meta) {
        ...
        $wallet->customer?->notify(new WalletTransactionNotification($transaction));
    ...
    public function debitForOrder(Customer $customer, Order $order, float $amount): WalletTransaction
    {
        ...
        $transaction = DB::transaction(function () use ($customer, $order, $amount) {
        ...
        $customer->notify(new WalletTransactionNotification($transaction));
